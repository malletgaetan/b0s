...
void io_apic_write_register(u8 offset, u32 val)
{
	...
	volatile_write32((u32 *)(io_apic_base + (u64)IO_APIC_REGISTER_DATA),
					 val); // what we want to write
}
...
void io_apic_init(void)
{
	struct acpi_madt_io_apic *io_apic =
		(struct acpi_madt_io_apic *)acpi_madt_find(ACPI_MADT_IO_APIC);
	if (io_apic == NULL)
		panic("%s: couldn't find IO APIC in acpi_madt", __func__);

	io_apic_base = (u64)vmm_map(kspace, (void *)DM_P2V((u64)io_apic->address),
								(void *)(u64)io_apic->address, PAGE_KERNEL_RW);
...
void local_apic_periodic_timer(void)
{
	...
	u32 reg = (rdmsr(LVT_TIMER_REGISTER) & ~(LVT_MASK)) | LVT_PERIODIC |
			  IDT_TIMER_INT; // mask off + vector 100
...
void local_apic_timer_calibrate(void)
{
	...
	wrmsr(DIVIDE_CONFIGURATION_REGISTER,
		  conf | TIMER_DIVIDE_BY_2); // divide frequency so we don't wrap around, even divide by 1
									 // is sufficient on my current computer but let's play it safe
...
void local_apic_init(void)
{
	...
	flags = flags | (1 << 10) |
			(1 << 11); // Intel Volume 3A: 10.12.1 Detecting and Enabling x2APIC Mode
