...
void hpet_sleep(u32 milliseconds) {
	u64 target = volatile_read64((u64 *)(base + HPET_MAIN_COUNTER_VALUE)) + (milliseconds * 1000000000000) / clock;
	while (volatile_read64((u64 *)(base + HPET_MAIN_COUNTER_VALUE)) < target); // HPET count up
}
...
void hpet_init(void) {
	...
	clock = volatile_read64((u64 *)(base + HPET_GENERAL_CAPABILITIES)) >> HPET_CAP_COUNTER_CLOCK_OFFSET;
