...
// and most flexible solution, but performance could be a lot better with a bit of optimizations or
// a buddy allocator
...
static char *type_debug[] = {
	[PMM_REGION_AVAILABLE] = "available", [PMM_REGION_RESERVED] = "reserved"};
...
{
	pmap[pmap_index].type =
		entry->type == MULTIBOOT_MEMORY_AVAILABLE ? PMM_REGION_AVAILABLE : PMM_REGION_RESERVED;

	if (pmap[pmap_index].type == PMM_REGION_AVAILABLE)
		pmap[pmap_index].addr =
			(void *)PAGE_ADDR(MAX(V2P(kernel_vma_stop), entry->addr),
							  PAGE_SIZE_IN_BYTES); // TODO: shouldn't use a MAX here
	else
	...
	pmap[pmap_index].size_in_pages =
		((u64)addr_stop - (u64)pmap[pmap_index].addr) / PAGE_SIZE_IN_BYTES;

	for (u32 i = 0; i < pmap_index; i++) { // check for possible overlap
		void *_start = pmap[i].addr;
		void *_stop =
			(void *)((u64)pmap[i].addr + (u64)(pmap[i].size_in_pages * PAGE_SIZE_IN_BYTES));
...
static void pmap_init(const struct multiboot_tag_mmap *mmap)
{
	...
	for (u32 i = 0; i < nb_entries; i++) {
		...
		u8 is_available_or_reserved = ((mmap->entries[i].type == MULTIBOOT_MEMORY_AVAILABLE) &&
									   (mmap->entries[i].addr >= BIOS_END)) ||
									  (mmap->entries[i].type == MULTIBOOT_MEMORY_RESERVED);
...
void pmm_dump_pmap(void)
{
	for (u64 i = 0; i < pmap_size; i++)
		printk("pa_start=%p | pa_stop=%p ~%uKiB | type=%s\n", pmap[i].addr,
			   (void *)((u64)pmap[i].addr + (pmap[i].size_in_pages * PAGE_SIZE_IN_BYTES)),
			   BYTE2KB(pmap[i].size_in_pages * PAGE_SIZE_IN_BYTES), type_debug[pmap[i].type]);
}
...
struct pmm_phys_addr pmm_next_phys_page(void *addr)
{
	...
	for (u32 i = 0; i < pmap_size; i++) {
		...
		if (next >= stop)
			continue;
		return (struct pmm_phys_addr){.pa = (void *)(MAX(next, (u64)pmap[i].addr)),
									  .type = pmap[i].type};
	}
...
u64 pmm_init(const struct multiboot_tag_mmap *mmap)
{
	...
	for (u64 pa = (u64)V2P(kernel_vma_stop); pa < (u64)V2P(pmm_vma_stop);
		 pa += PAGE_SIZE_IN_BYTES) {
		(void)pmm_alloc(); // TODO: could be dangerous if we change the pmm_alloc implementation,
						   // which currently return first free page
		--available_pages;
	}
